...
> **Webcam users:** the base requirements are sized for serverless deploys. To use a local USB webcam, also install `opencv-python`:
> ```bash
...
The dashboard is serverless-ready:

- `pyproject.toml` points Vercel to `dashboard:app`.
- `vercel.json` routes all requests into the FastAPI app.
- `requirements.txt` uses the lightweight `ai-edge-litert` interpreter instead of full TensorFlow, and omits the optional `opencv-python` webcam dependency.
- On Vercel, SQLite and snapshots write to `/tmp` automatically.

Upload the repository to a GitHub project, import it into Vercel, and set the framework preset to **Other**. The live camera and trap pipeline do not run on Vercel (no camera/hardware), but the dashboard UI, photo upload/classify endpoint, chatbot, and detection history all work.
