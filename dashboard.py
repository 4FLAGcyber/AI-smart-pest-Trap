...
# Vercel serverless allows writes only under /tmp; keep local logs/ path for dev.
DB_PATH = "/tmp/dashboard.db" if config.IS_VERCEL else os.path.join(BASE_DIR, "logs", "dashboard.db")
...
# Model is loaded lazily on the first classification request so import-time
# startup stays fast and lightweight for serverless environments.
...
def init_db():
    try:
        ...
        print(f"[db] Could not open file database ({e}); falling back to in-memory.")
        global DB_PATH
        DB_PATH = ":memory:"
        get_db()
...
@app.get("/api/health")
def health():
    """Lightweight liveness check that does not load the AI model."""
    return {
        "ok": True,
